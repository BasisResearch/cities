...
state_index = torch.tensor(
    [0, 0, 0, 1, 1, 1, 2, 2, 2, 3], dtype=torch.int, device=device
).repeat(10)
...
def cities_model_E(
    ...
    weight_TY = pyro.sample("weight_TY", dist.Normal(0, 0.5))
    ...
    with time_plate:
        ...
        Ut = pyro.sample("Ut", dist.Normal(0, 0.4))

        # impact thereof on treatment
        weight_UtT = pyro.sample("weight_UtT", dist.Normal(0, 0.4))

        # impact thereof on outcome
        weight_UtY = pyro.sample("weight_UtY", dist.Normal(0, 0.4))

    with states_plate:
        # state-related confounders
        Us = pyro.sample("Us", dist.Normal(0, 0.4))

        # impact thereof on treatment
        weight_UsT = pyro.sample("weight_UsT", dist.Normal(0, 0.4))

        # impact thereof on outcome
        weight_UsY = pyro.sample("weight_UsY", dist.Normal(0, 0.4))

        with covariates_plate:
            weight_UsX = pyro.sample("weight_UsX", dist.Normal(0, 0.4))

    with covariates_plate:
        X_bias = pyro.sample("X_bias", dist.Normal(0, 1))
        weight_XT = pyro.sample("weight_XT", dist.Normal(0, 0.4))
        weight_XY = pyro.sample("weight_XY", dist.Normal(0, 0.4))

    with counties_plate:
        UsX_weighted = torch.einsum("ij...,j...->ij", weight_UsX, Us)
        UsX_weighted_selected = UsX_weighted[..., state_index_sparse].squeeze()
        X_means = torch.einsum("i...,ik->ik", X_bias, UsX_weighted_selected)[
            ..., unit_index_sparse
        ].T
        X = pyro.sample("X", dist.Normal(X_means, 0.4))
    ...
    with observations_plate:
        ...
        T_mean = (
            T_bias
            + UtT_weighted[time_index, ...].squeeze()
            + UsT_weighted[state_index, ...].squeeze()
            + XT_weighted[unit_index]
        )
        print("T_mean", T_mean.shape)

        # TODO add prior over this variance
        T = pyro.sample("T", dist.Normal(T_mean, 0.4))
        ...
        Y_mean = (
            Y_bias
            + UtY_weighted[time_index, ...].squeeze()
        ...
        # TODO add prior over this variance
        Y = pyro.sample("Y", dist.Normal(Y_mean, 0.4))


with pyro.poutine.trace() as tr:
    cities_model_E(
        N_t, N_s, state_index, time_index, intervention, covariates_sparse, observed_y=y
    )

pyro.render_model(
    cities_model_E,
    model_args=(N_t, N_s, state_index, time_index, intervention, covariates_sparse),
    render_distributions=True,
    filename="docs/experimental_notebooks/cities_model_E.png",
)
