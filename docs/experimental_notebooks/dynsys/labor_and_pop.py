...
class TotalLaborForceDynamics(FluxDynamics):
    ...
    def diff(self, dstate: State[torch.Tensor], state: State[torch.Tensor]) -> None:
        ...
        t = state['t'].detach().requires_grad_(True)
        dtlfdt = torch.autograd.functional.jacobian(self.tlf, t) # .shape == (n_fips,)
        ...
        # Broadcast the total labor force delta across the flux proportions for each of the categories.
        dstate["fluxers"] += normed_lf_fluxes * dtlfdt[None, :]
...
def flux_prior(n_fips, n_states, n_cats, mapped_state_idx):

    src_cat_plate = pyro.plate("src_cat_plate", n_cats, dim=-4)
    dst_cat_plate = pyro.plate("dst_cat_plate", n_cats, dim=-3)
    ...
    with src_cat_plate:
        with dst_cat_plate:
            # TODO mask diagonal along categories.
    ...
    return transform_county_fluxes(county_flux, end_scale), src_cat_plate, dst_cat_plate, states_plate, counties_plate
...
def full_prior(employment_classes, fips_codes):
    ...
    fluxes, src_cat_plate, dst_cat_plate, states_plate, counties_plate = flux_prior(
        n_fips=len(fips_codes),
        n_states=len(unique_state_codes),
        n_cats=len(employment_classes),
    ...
    with src_cat_plate:
        with counties_plate:
            initial_state = State(
                fluxers=pyro.sample('fluxers0', dist.Uniform(0.5, 1.0))  # .shape == (n_cats, n_fips)
                .squeeze(dst_cat_plate.dim, states_plate.dim)  # TODO HACK eh...might be worth making new plate?
            )

            # The fixed proportions of labor force growth that go to each of the classes.
            # Normalization is handled downstream. Total labor force growth is defined below.
    ...
    fdyns = TotalLaborForceDynamics(
        flux_rates=fluxes,
        total_labor_force_f_of_t=lambda t: tlf_start * torch.exp(lf_growth * t),
        lf_fluxes=lf_fluxes,
        noise_scale=.05
    )
...
def pop_dynsys_only():
    ...
    for fc in range(len(fips_codes)):
        plt.figure()
        for i in range(len(employment_classes)):
            plt.plot(traj.times, traj.trajectory["fluxers"][i, fc], color=colors[employment_classes[i]])

        # Also plot the sum of the fluxers for each fc, alongside the tlf curve.
        plt.plot(traj.times, traj.trajectory["fluxers"][:, fc].sum(dim=0), color="black")
        plt.plot(traj.times, torch.stack([fdyns.tlf(t)[fc] for t in traj.times]), color="red", linestyle="--")

        plt.ylim(0, len(employment_classes))
