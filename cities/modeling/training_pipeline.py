...
logging.info(f"{(num_files-2)/2} guides already exist. Starting to train {N_combinations - (num_files -2)/2} out of {N_combinations} guides needed.")
...
for intervention_dataset in interventions:
    for outcome_dataset in outcomes:
        for forward_shift in shifts:
            ...
            guide_path = os.path.join(root, "data/model_guides", f"{guide_name}_guide.pkl")
