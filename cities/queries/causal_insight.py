...
class CausalInsight():
    ...
    def load_guide(self, forward_shift):
        ...
        guide_name = (f"{self.intervention_dataset}_{self.outcome_dataset}_{forward_shift}")
        guide_path = os.path.join(self.root, "data/model_guides", f"{guide_name}_guide.pkl")
    ...
    def generate_tensed_samples(self):
        ...
        for shift in [1,2,3]:
            ...
            self.tensed_tau_samples[shift] = self.samples['weight_TY'].squeeze().detach().numpy()
    ...
    def get_fips_predictions(self, fips, intervened_value, year=None):
        ...
        self.fips_id =  dg.std_wide[self.intervention_dataset].loc[
               dg.std_wide[self.intervention_dataset]['GeoFIPS'] == fips].index[0]
        
        self.name = dg.std_wide[self.intervention_dataset]["GeoName"].iloc[self.fips_id]
        

        # get observed values at the prediction times
        self.observed_intervention = dg.std_wide[self.intervention_dataset].iloc[self.fips_id][str(year)]
        self.observed_outcomes = dg.std_wide[self.outcome_dataset].iloc[self.fips_id][outcome_years[year_id:(year_id+4)]]
        ...
        for shift in [1,2,3]:
            self.intervention_impact[shift] = self.tensed_tau_samples[shift] * self.intervention_diff
            self.intervention_impact_mean.append(np.mean(self.intervention_impact[shift]))
            self.intervention_impact_low.append(np.percentile(self.intervention_impact[shift], 2.5))
            self.intervention_impact_high.append(np.percentile(self.intervention_impact[shift], 97.5))

        predicted_mean = [self.observed_outcomes.iloc[0]] + (self.intervention_impact_mean + self.observed_outcomes.iloc[1:]).tolist()
        predicted_low = [self.observed_outcomes.iloc[0]] + (self.intervention_impact_low + self.observed_outcomes.iloc[1:]).tolist()
        predicted_high = [self.observed_outcomes.iloc[0]] + (self.intervention_impact_high + self.observed_outcomes.iloc[1:]).tolist()

        self.predictions = pd.DataFrame({"year": self.prediction_years, "observed": self.observed_outcomes, "mean": predicted_mean, "low": predicted_low, "high": predicted_high})
...
# look into this some time, do this by hand for now
    ...
    def plot_predictions(self, range_multiplier=1.5):
        ...
        y_min = 1.5 * min(self.fips_observed_data["Value"].min(), self.predictions["low"].min())
        y_max = 1.5 * max(self.fips_observed_data["Value"].max(), self.predictions["high"].max())  
        ...
        title = f"{self.outcome_dataset} in {self.name} under intervention {self.intervened_value} in year {self.year}" \
            f"compared to the observed values under observed intervention {self.observed_intervention}."
    ...
    def plot_residuals(self):
        ...
        plt.hist(self.average_predictions - self.data['y'].squeeze(),
          bins=70)
        plt.show()
    ...
    def predictive_check(self):
        ...
        plt.title(f"{self.intervention_dataset}, {self.outcome_dataset}." 
                  f"R2={r_squared}")
        #TODO round r2
        plt.show
    ...
    def estimate_ATE(self):
        ...
        plt.title(f"ATE for {self.intervention_dataset}  and  {self.outcome_dataset} with forward shift = {self.forward_shift}")
        plt.legend()
        plt.show()
