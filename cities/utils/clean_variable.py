...
class VariableCleanerMSA(VariableCleaner): # this class inherits functionalites of VariableCleaner, but its adjusted to MSA level
    ...
    def save_csv_files(self):
        # wrangling
        variable_db_wide = self.variable_db.copy()
        variable_db_long = pd.melt(self.variable_db, id_vars=["GeoFIPS", "GeoName"], var_name=self.YearOrCategory,
                                   value_name="Value")
        variable_db_std_wide = standardize_and_scale(self.variable_db)
        variable_db_std_long = pd.melt(variable_db_std_wide.copy(), id_vars=["GeoFIPS", "GeoName"],
                                       var_name=self.YearOrCategory, value_name="Value")
        # saving
        variable_db_wide.to_csv((f"{self.root}/data/MSA_level/" + self.variable_name + "_ma_wide.csv"), index=False)
        variable_db_long.to_csv((f"{self.root}/data/MSA_level/" + self.variable_name + "_ma_long.csv"), index=False)
        variable_db_std_wide.to_csv((f"{self.root}/data/MSA_level/" + self.variable_name + "_ma_std_wide.csv"),
                                    index=False)
        variable_db_std_long.to_csv((f"{self.root}/data/MSA_level/" + self.variable_name + "_ma_std_long.csv"),
                                    index=False)
