...
def clean_ethnic_initially():
    ...
    ethnic_composition = ethnic_composition[ethnic_composition['CBSA'].isin(metro_areas['GeoFIPS'])]

    ethnic_composition = pd.merge(ethnic_composition, metro_areas[['GeoFIPS', 'GeoName']], left_on='CBSA', right_on='GeoFIPS', how='inner')
    ...
    ethnic_composition_ma = ethnic_composition[['GeoFIPS', 'GeoName'] + list(ethnic_composition.columns[0:cols_to_save])]
    ...
    ethnic_composition_ma.to_csv(f"{root}/data/raw/ethnic_composition_ma.csv", index=False)
