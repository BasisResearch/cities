...
def assert_no_repeated_non_ones(tensor_size):
    ...
    assert len(non_ones) == len(set(non_ones)), f"Repeated non-1 dimensions found in {tensor_size}"


@pytest.mark.parametrize("use_plate, use_mwc, use_do", [
    (True, True, True),
    (True, True, False),
    (True, False, True),
    (False, True, True),
])
@pytest.mark.parametrize("model_class, data", [
    (TractsModel, subset),
    ])
def test_plated_sample_shaping(use_plate, use_mwc, use_do, model_class, data):

    model = model_class(
        **data, categorical_levels=ct_dataset_read.categorical_levels
    )
    ...
    pyro.clear_param_store()
    guide = run_svi_inference(
    model, n_steps=n_steps, lr=0.03, plot=False, **data
    )
    ...
    if use_do:
        context_managers.append(do(actions={"limit": (torch.tensor(0.0), torch.tensor(1.0))}))
    ...
    assert samples_conditioned_contextualized['housing_units'].shape == torch.Size([num_samples, n])
