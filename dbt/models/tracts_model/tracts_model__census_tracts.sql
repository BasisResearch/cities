...
population as (select * from {{ ref('census_tracts_population') }}),
demographics as (select * from {{ ref('demographics') }}),
census_tracts as (select * from {{ ref('tracts_model_int__census_tracts_filtered') }}),

-- Demographic data
white as (select * from demographics where name_ = 'B03002_003E'), -- white non-hispanic population
income as (select * from demographics where name_ = 'B19013_001E'), -- median household income
segregation as (
select * from demographics where description = 'segregation_index_annual_city'
),
...
select
  ...
  {{ safe_divide('white.value_', 'population.total_population') }} as white,
...
from
  ...
  inner join population using (census_tract_id)
  left join segregation using (census_tract, year_)
  left join white using (census_tract, year_)
